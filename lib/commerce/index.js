...
export async function commerceFindOrderByIdempotencyKey(idempotencyKey) {
  if (!idempotencyKey) return null;
  const db = await getCommerceBackend();
  if (typeof db.findOrderByIdempotencyKey === 'function') {
    return db.findOrderByIdempotencyKey(idempotencyKey);
  }
  const orders = await db.listOrders();
  return orders.find((o) => o.idempotency_key === idempotencyKey) || null;
}
...
export async function commerceListFulfillmentJobs(opts) {
  const db = await getCommerceBackend();
  return db.listFulfillmentJobs(opts);
}

export async function commerceListWebhookEvents(opts) {
  const db = await getCommerceBackend();
  return db.listWebhookEvents(opts);
}

export async function commerceListAuditLog(opts) {
  const db = await getCommerceBackend();
  return db.listAuditLog(opts);
}

export async function commerceGetBackendName() {
  const db = await getCommerceBackend();
