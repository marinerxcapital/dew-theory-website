...
async function loadFeedText(feedUrl) {
  if (!feedUrl) {
    const err = new Error(
      'SKIN_SCRIPT_FEED_URL is required for csv_feed mode. Point it at an authorized wholesale CSV/JSON export. Sync will not fall back to mock.'
    );
  ...
  if (feedUrl.startsWith('http://') || feedUrl.startsWith('https://')) {
    ...
    if (!res.ok) {
      const err = new Error(`Authorized feed HTTP ${res.status}`);
  ...
  if (feedUrl.startsWith('file:')) {
    const err = new Error('file: URLs are not supported — use an absolute or project-relative path');
    err.code = 'feed_url_invalid';
    throw err;
  }
