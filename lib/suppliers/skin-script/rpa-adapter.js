/**
 * Skin Script RPA adapter — delegates fulfillment + allowlisted catalog to private Playwright service.
 * SKIN_SCRIPT_MODE=rpa
 */
import { signedFetch } from '../../internal/hmac-auth.js';
import { logInfo, logWarn } from '../../log.js';
import { allowlistCatalogRequestItems } from '../../catalog-allowlist.js';
  ...
  return {
    ...
    async listCatalog() {
      if (!rpaEnabled(env) || !serviceConfigured(env)) {
        const err = new Error(
          'RPA catalog requires SKIN_SCRIPT_RPA_ENABLED=true plus SKIN_SCRIPT_RPA_SERVICE_URL and SKIN_SCRIPT_RPA_HMAC_SECRET. Will not fall back to mock.'
        );
        err.code = 'rpa_not_configured';
        throw err;
      }
      const items = allowlistCatalogRequestItems();
      const res = await signedFetch('/v1/catalog/list', {
        method: 'POST',
        body: { items }
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        const err = new Error(data.detail || data.error || `RPA catalog list failed: ${res.status}`);
        err.code = data.code || data.error_code || 'rpa_catalog_failed';
        throw err;
      }
