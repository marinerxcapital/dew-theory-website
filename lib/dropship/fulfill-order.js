...
export async function fulfillOrder(orderId, opts = {}) {
  ...
  let job = await commerceGetFulfillmentJobByOrder(id);
  if (job?.supplier_order_id && !opts.force) {
    return { ok: true, order, idempotent: true, code: 'already_submitted' };
  }
  ...
  const ensured = await ensureFulfillmentJobForPaidOrder(order);
  job = ensured?.job || job || (await commerceGetFulfillmentJobByOrder(id));
  ...
  if (isRpaProduction) {
    ...
    if (missing.length) {
      ...
      const updated = await markFulfillmentFailureAsync(id, msg, 'sku_missing', opts.adminId, job?.id, null, opts);
  ...
  if (!lines.length) {
    ...
    const updated = await markFulfillmentFailureAsync(id, msg, 'empty_lines', opts.adminId, job?.id);
  ...
  try {
    ...
    if (job?.id && result.status !== 'dry_run_ready') {
...
async function markFulfillmentFailureAsync(orderId, message, code, adminId, jobId, classification, opts = {}) {
  ...
  if (!after) {
    const durable = await commerceGetOrder(orderId);
    if (durable) after = { ...durable, ...patch };
  }
  ...
  if (jobId) {
    ...
    // Job helper also patches the order (RPA blocked statuses). Re-apply the
    // mode-aware order patch so mock stays failed_supplier, not a blocked_* state.
 ...
 * Kept in sync with computeAutoFulfillEnabled() in lib/admin/dashboard.js.
 * @param {NodeJS.ProcessEnv | Record<string, string|undefined>} [env]
 */
export function shouldAutoFulfill(env = process.env) {
