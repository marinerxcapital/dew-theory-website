...
const DEV_EMAIL = 'admin@dewtheory.local';
const DEV_PASSWORD = 'dew-admin-dev';
const DEV_SESSION_SECRET = 'dew-theory-dev-secret-change-me';
...
export function validateCredentials(email, password) {
  ...
  if (isProduction()) {
    ...
    if (envPass === DEV_PASSWORD) return null;
    if (!sessionSecret()) return null;
  }

  const expectedEmail = (envEmail || DEV_EMAIL).toLowerCase();
  const expectedPass = envPass || DEV_PASSWORD;

  const emailOk = timingSafeEqualString(String(email || '').toLowerCase(), expectedEmail);
  const passOk = timingSafeEqualString(String(password || ''), expectedPass);
  if (!emailOk || !passOk) return null;

  const store = readStore();
  return store.admins.find((a) => a.email.toLowerCase() === expectedEmail) || store.admins[0] || null;
}
