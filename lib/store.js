...
import { getAllowlistSkuForProductId } from './catalog-allowlist.js';
...
function seed() {
  ...
  return {
    products: catalog.products.map((p) => ({
      ...
      skin_script_sku:
        p.skin_script_sku ||
        getAllowlistSkuForProductId(p.id) ||
    ...
    discount_codes: [
      {
        ...
        type: 'percentage',
        value: 15,
        // Launch-promo placeholder only — 15% is not an Emily-confirmed rate.
        // See OPEN_ITEMS.md §2 (2026-09-19 confirmation still pending).
        note: 'Launch promo placeholder — 15% pending Emily confirmation',
...
function heal(data) {
  ...
  if (!data.catalog_sync || typeof data.catalog_sync !== 'object' || Array.isArray(data.catalog_sync)) {
    data.catalog_sync = null;
  }
  return data;
}
