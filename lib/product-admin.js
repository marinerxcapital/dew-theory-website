...
/**
 * Honesty flags default false unless explicitly true.
 * Admin/CSV/sync writes must not promote an unconfirmed catalog value to confirmed.
 */
export function parseHonestyFlag(value, defaultValue = false) {
...
export function validateAndNormalizeProduct(body, opts = {}) {
  ...
  const product = {
    ...
    retail_price_confirmed: parseHonestyFlag(b.retail_price_confirmed, false),
  ...
  if (Object.prototype.hasOwnProperty.call(b, 'size_confirmed')) {
    product.size_confirmed = parseHonestyFlag(b.size_confirmed, false);
  }
  if (b.size_note) product.size_note = String(b.size_note).slice(0, 2000);
  if (b.retail_price_note) product.retail_price_note = String(b.retail_price_note).slice(0, 2000);
  if (b.manufacturer_name_note) {
    product.manufacturer_name_note = String(b.manufacturer_name_note).slice(0, 2000);
  }
